    ...
    >
      <TabsList className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mb-8">
        {services.map((service) => (
          <TabsTrigger key={service.id} value={service.id} className="text-sm md:text-base py-3">
            {service.title}
          </TabsTrigger>
        ))}
      </TabsList>
      ...
      {services.map((service) => (
        <TabsContent key={service.id} value={service.id} className="space-y-6 animate-fadeIn">
          <div className="grid md:grid-cols-2 gap-8">
            ...
            {service.caseStudy && (
              <div className="glass p-6 rounded-lg space-y-4">
                ...
                <h5 className="text-lg font-medium">{service.caseStudy.title}</h5>
