...
export function HeroSection({ title, subtitle, children, className, imageUrl, fullWidth = false }: HeroSectionProps) {
  return (
    <section className={cn("relative min-h-[90vh] flex items-center justify-center overflow-hidden", className)}>
      ...
      {/* Content */}
      <div className={cn("container relative z-10 px-4 py-24 md:py-32", fullWidth && "max-w-none")}>
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          {children && <div className="mt-10 animate-fadeIn animation-delay-300">{children}</div>}
        </div>
      </div>
    </section>
  )
